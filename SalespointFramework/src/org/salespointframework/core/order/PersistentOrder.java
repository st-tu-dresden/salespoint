...
public class PersistentOrder implements Order<PersistentOrderLine, PersistentChargeLine> {
	...
	public final OrderCompletionResult completeOrder() {
		// TODO mehr CompletionStates
		if(orderStatus == OrderStatus.CANCELLED) return new OrderCompletionResultImpl(OrderCompletionStatus.FAILED, null, null, null);
		if(orderStatus == OrderStatus.COMPLETED) return new OrderCompletionResultImpl(OrderCompletionStatus.FAILED, null, null, null);
		if(orderStatus == OrderStatus.PAYED) return new OrderCompletionResultImpl(OrderCompletionStatus.FAILED, null, null, null);
		
		Inventory<?> tempInventory = Shop.INSTANCE.getInventory();
		if(!(tempInventory instanceof PersistentInventory)) throw new RuntimeException("Sorry, PersistentInventory only :(");	// TODO Exception Name
		...
		for(PersistentOrderLine orderLine : orderLines) {
			...
			for(PersistentProduct product : products) {
				...
				if(!result) {
					...
					return new OrderCompletionResultImpl(OrderCompletionStatus.SPLITORDER, order, em, null);
		...
		try {
			...
			return new OrderCompletionResultImpl(OrderCompletionStatus.FAILED, null, em, e.getCause());
		}
		
		return new OrderCompletionResultImpl(OrderCompletionStatus.SUCCESSFUL, null, em, null);
	}
	...
	public boolean payOrder(/* TODO PaymentMethod paymentMethod */) {
		if(!(orderStatus == OrderStatus.PAYED)) return false;;
	...
	private final class OrderCompletionResultImpl implements OrderCompletionResult {
		...
		public OrderCompletionResultImpl(OrderCompletionStatus orderCompletionStatus, PersistentOrder order, EntityManager entityManager, Throwable exception) {
		...
		public final Order splitOrder() {
			if(this.orderCompletionStatus == OrderCompletionStatus.SPLITORDER) {
				if(this.entityManager.getTransaction().isActive()) {
					try {
						this.entityManager.getTransaction().commit();	// TODO Commit Fails -> Rollback
					}
