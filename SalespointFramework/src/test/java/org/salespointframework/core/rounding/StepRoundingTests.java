 ...
 * than, equal and greater than the step. Also the value is tested as multiple
 * of step, again smaller than n*step (but greater than (n-1)*step, equal to
 * n*step and greater than n*step (but smaller than (n+1)*step). Also a step
 * value of greater 1 and between 0 and 1 is tested, plus a negative and
 * positive value and a positive and negative step value. This gives 4
 * (direction) * (3 (size) + 3 (size, multiple)) * 2 (fractional step or not) *
 * 2 (sign) * 2 (step sign) = 192 tests.
 * 
 * Thus, as single method executes all tests.
 * 
...
public class StepRoundingTests {
	...
	static List<BigDecimal> steps = Arrays.asList(BigDecimal.valueOf(50),
			BigDecimal.valueOf(-50), BigDecimal.valueOf(0.005),
	...
	static public void setup() {
		...
		for (RoundingMode mode : modes) {
			...
			for (BigDecimal step : steps) {
				...
				/* single, negative */
				result.put(step.multiply(BigDecimal.valueOf(0.95)).negate(),
						step.negate());
				result.put(step.multiply(BigDecimal.valueOf(1.05)).negate(),
						step.add(step).negate());

				/* multiple, negative */
				result.put(multipleStep.multiply(BigDecimal.valueOf(0.95))
						.negate(), multipleStep.negate());
				result.put(multipleStep.multiply(BigDecimal.valueOf(1.05))
						.negate(), multipleStep.add(step).negate());

				/* single, positive */
				result.put(step.multiply(BigDecimal.valueOf(0.95)), step);
				result.put(step.multiply(BigDecimal.valueOf(1.05)),
						step.add(step));
				...
				case CEILING:
					/* single, negative */
					result.put(
							step.multiply(BigDecimal.valueOf(0.95)).negate(),
							BigDecimal.ZERO);
					result.put(
							step.multiply(BigDecimal.valueOf(1.05)).negate(),
							step.negate());

					/* multiple, negative */
					result.put(multipleStep.multiply(BigDecimal.valueOf(0.95))
							.negate(), multipleStep.subtract(step).negate());
					result.put(multipleStep.multiply(BigDecimal.valueOf(1.05))
							.negate(), multipleStep.negate());
					break;
				case DOWN:
					/* single, positive */
					result.put(step.multiply(BigDecimal.valueOf(0.95)),
							BigDecimal.ZERO);
					...
					/* multiple, positive */
					result.put(multipleStep.multiply(BigDecimal.valueOf(0.95)),
							multipleStep.subtract(step));
					result.put(
							step.multiply(BigDecimal.valueOf(6)).multiply(
									BigDecimal.valueOf(1.05)), multipleStep);
				...
				for (Entry<BigDecimal, BigDecimal> e : result.entrySet()) {
					System.out.println(mode + ": " + " for step " + step
							+ " adding <K,V>: <" + e.getKey() + ","
							+ e.getValue() + ">.");
				}
	...
	private String msg(BigDecimal value, BigDecimal step, RoundingMode mode,
			BigDecimal result) {
		return "Rounding value " + value + " with step " + step
				+ " in direction " + mode + " with result: " + result;
	...
	public void newTest() {
		RoundingMode mode;
		BigDecimal result;
		for (Entry<RoundingMode, Map<BigDecimal, Map<BigDecimal, BigDecimal>>> test : tests
				.entrySet()) {
			mode = test.getKey();
			for (Entry<BigDecimal, Map<BigDecimal, BigDecimal>> e : test
					.getValue().entrySet()) {
				BigDecimal step = e.getKey();
				for (Entry<BigDecimal, BigDecimal> pair : e.getValue()
						.entrySet()) {
					result = new BasicRoundingStrategy(step, mode)
							.round(pair.getKey());
					assertThat(msg(pair.getKey(), step, mode, result), result,
							is(equal(pair.getValue())));
				}
			}
	...
	private void testSmallerSingle(BigDecimal value, BigDecimal step,
			RoundingMode mode, BigDecimal result) {
		...
		if (step.compareTo(BigDecimal.ZERO) < 0) {
			assertThat(
					msg,
					result,
					allOf(is(smallerThanOrEqual(BigDecimal.ZERO)),
							is(greaterThan(step.multiply(BigDecimal.valueOf(2))))));
		} else {
			assertThat(
					msg,
					result,
					allOf(is(greaterThanOrEqual(BigDecimal.ZERO)),
							is(smallerThan(step.multiply(BigDecimal.valueOf(2))))));
		}
	...
	public void StepTests() {
		...
		for (RoundingMode mode : modes) {
			for (BigDecimal step : steps) {
				BigDecimal smaller = step.subtract(step.divide(BigDecimal
						.valueOf(2)));
				BigDecimal greater = step
				...
				assertThat(remainder, is(zero()));
				msg = "Rounding value " + step + " with step " + step
						+ " in direction " + mode + " with result: " + result
				...
				assertThat("remainder is not zero", remainder, is(zero()));
				assertThat(remainder, is(zero()));
				msg = "Rounding value " + greater + " with step " + step
						+ " in direction " + mode + " with result: " + result
						+ " and remainder of division: " + remainder;
				...
				if (step.compareTo(BigDecimal.ZERO) < 0) {
					assertThat(
							msg,
							result,
							allOf(is(smallerThanOrEqual(step)),
									is(greaterThan(step.multiply(BigDecimal
											.valueOf(3))))));
				} else {
					assertThat(
							msg,
							result,
							allOf(is(greaterThanOrEqual(step)),
									is(smallerThan(step.multiply(BigDecimal
											.valueOf(3))))));
				}
