...
public interface Accountancy {
	 ...
	 * {@code clazz} and all sub-types, previously added to the
	 * accountancy.
	 * 
	 * If no entries of the specified type exist, an empty {@code Iterable}
	 * is returned.
	 * 
	 * @param <E>
	 ...
	 * 
	 * {@literal null} is returned, if no entry with the given identifier
	 * exists.
	 * 
	 * @param <E>
	 *            common super type of all entries returned
	 ...
	 * {@code from} and {@code to} of the specified class type
	 * {@code clazz} and all sub-types, including from and to. So every
	 * entry with an time stamp <= {@code to} and >= {@code from} is
	...
	/**
	 * Returns all {@link AccountancyEntry}s of type {@code clazz} and
	 * all sub-types, which have their {@code date} within (including)
	 * {@code from} and {@code to}. <br />
	 * The time between {@code from} and {@code to} is divided into
	 * parts of {@code period} length. According to their respective date,
	 * entries are sorted in exactly one of the time intervals. The last time
	 * interval may be shorter than {@code period}.<br />
	 ...
	 *         {@code from} and {@code to} as keys, and as value an
	 ...
	 * {@link AccountancyEntry}s of type {@code clazz} and its
	 * sub-types, which have their {@code date} within (including)
	 * {@code from} and {@code to}. <br />
	 * The time between {@code from} and {@code to} is divided into
	 * parts of {@code period} length. According to their time stamp,
	 * entries are sorted in exactly one of the time intervals. The last time
	 * interval may be shorter than {@code period}.<br />
	 ...
	 * equal to the sum of all entries' {@code amount}-field, with a date
	 ...
	 *            {@code from} and {@code to} is divided
	 * @return a map, with intervals of {@code period} length between
	 *         {@code from} and {@code to} as keys, and as value a
