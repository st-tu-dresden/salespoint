...
An \code{Order} can be imagined as sheet of paper which basically consists of lines representing the ordered products.
...
Every product of an order is stored in separate \code{OrderLine}.
An \code{OrderLine} is uniquely identified by an \code{OrderLineIdentifier}.
\code{OrderLine}s contain all information to identify product instances, like a \code{ProductType} and \code{ProductFeatures}.
\\

\code{ChargeLine}s represent additional costs or discounts and can be applied to \code{OrderLine}s or \code{Order}s. 
...
The lifecycle covers four states which are defined by enumeration type \code{OrderEntryStatus}.
...
\begin{itemize}
\item \code{SUCCESSFULL}: The order was completed successfully, and all products were removed from the inventory. 
