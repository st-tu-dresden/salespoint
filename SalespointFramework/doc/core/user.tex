...
The reason for this behaviour is, that data is not stored inside the \code{PersistentUserManager} object itself, as it may be done with a collection-based implementation, but \code{PersistentUserManager} is merely a transparent interface to the JPA.\\
