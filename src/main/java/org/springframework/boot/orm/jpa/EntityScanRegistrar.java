...
class EntityScanRegistrar implements ImportBeanDefinitionRegistrar {
	...
	public void registerBeanDefinitions(AnnotationMetadata importingClassMetadata, BeanDefinitionRegistry registry) {
		...
		if (registry.containsBeanDefinition(BEAN_NAME)) {
			...
			beanDefinition.getConstructorArgumentValues().addGenericArgumentValue(getPackagesToScan(importingClassMetadata));
	...
	private String[] getPackagesToScan(AnnotationMetadata metadata) {
		AnnotationAttributes attributes = AnnotationAttributes.fromMap(metadata.getAnnotationAttributes(EntityScan.class
				.getName()));
		...
		return new ArrayList<String>(packagesToScan).toArray(new String[packagesToScan.size()]);
	}

	/**
	 * {@link BeanPostProcessor} to set {@link LocalContainerEntityManagerFactoryBean#setPackagesToScan(String...)} based
	...
	static class EntityScanBeanPostProcessor implements BeanPostProcessor, SmartInitializingSingleton, Ordered {
		...
		public void afterSingletonsInstantiated() {
			Assert.state(this.processed, "Unable to configure " + "LocalContainerEntityManagerFactoryBean from @EntityScan, "
