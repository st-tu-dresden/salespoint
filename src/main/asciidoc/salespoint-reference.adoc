...
Be sure to check that the additional attributes do now shadow properties of the parent class.
...
3. _In your application code, use `MyCatalog` instead of `Catalog`._ -- This possible as `MyCatalog` inherits all basic CRUD functionality from `Catalog`.
...
2. _Create a custom `MyOrderRepository` extending Spring Data's `org.springframework.data.repository.Repository` interface._ -- Type it to `MyOrder` (domain type) and `OrderIdentifier` (identifier type).
