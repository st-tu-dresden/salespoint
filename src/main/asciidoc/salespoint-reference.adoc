...
* https://github.com/st-tu-dresden/salespoint/issues/225[#225] – Upgrade to Spring Boot 2.1 GA
* https://github.com/st-tu-dresden/salespoint/issues/218[#218], https://github.com/st-tu-dresden/salespoint/issues/219[#219] – Added the ability to delete both `UserAccount` and `Order` instances.
Note that when deleting the former, *all other* aggregate that keep a reference to the `UserAccount` have to be deleted manually first, e.g. all orders associated with this account.
...
That means you mandatorily need to provide an email when initially registering, i.e. you'll need to use the overload of `UserAccountManager.create(…)` that takes an email address as parameter.
